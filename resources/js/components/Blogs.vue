...
<div class="modal fade " id="addNew" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="addNewLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form @submit.prevent="editMode?update_blog():create_blog()">
      <div class="modal-body">
        ...
        <div class="form-group">
        <label> Content </label>
        <textarea v-model="form.content" rows="15" name="content"
            class="form-control" :class="{ 'is-invalid': form.errors.has('content') } " required>
         <div v-if="form.errors.has('content')" v-html="form.errors.get('content')" /></textarea>
        </div>

        <div class="form-group">
        <label> Image  </label>
        <input @change="upload" type="file" name="image"
            class="form-control" :class="{ 'is-invalid': form.errors.has('image') } " :required="!editMode">
...
<!--begin: Datatable-->
        <table class="table table-separate table-head-custom table-checkable" id="kt_datatable1">
            ...
            <tbody>
                ...
                <tr v-for="blog in blogs.data" :key="blog.index" v-else>
                    <td>{{blog.id}}</td>
                    <td>{{blog.title}}</td>
                    
                    <td>
                        <div  v-if="blog.image!=null">
                     <img  :src="'/uploads/'+blog.image" class="img-small">
                        </div>
                     <div v-else> <img src="/assets1/media/users/blank.png" class="img-small"> </div>
                    </td>
                    <td>{{blog.content | shortcut }}</td>
                  
                    <td>{{blog.status}}</td>
                    
                    <td  class="no-print2">
                        <span class="dtr-data">         
                            <a @click="editModal(blog)" href="javascript:;" class="btn btn-sm btn-clean btn-icon btn-primary" title="Edit">
                                <i class="la la-edit"></i>
                            </a>
                            <a  @click="delete_blog(blog.id)" href="javascript:;" class="btn btn-sm btn-clean btn-icon btn-danger" title="Delete">
       ...
       methods:{
           ...
           editModal(blog){
            ...
            create_blog(){
                ...
                this.$Progress.start() 
                this.form.post('/admin/api/blogs') //Send Post Request
                .then(()=>{ 
                     $('#addNew').modal('hide'); //Hide Model 
                     toast.fire({
                      icon: 'success',
                      title: 'Success.',  ///Toast Notifiblogion
                     })
            ...
            update_blog(){
                ...
                this.form.put('/admin/api/blogs/'+this.form.id) //Send Post Request
                .then(()=>{ 
                     $('#addNew').modal('hide'); //Hide Model 
                     toast.fire({
                      icon: 'success',
            ...
            delete_blog(id){
                ...
                swal.fire({
                  ...
                  confirmButtonText: 'Yes , Delete '
                    ...
                    this.form.delete('/admin/api/blogs/'+id)
                        .then(()=>{
                     swal.fire(
           ...
           searching(){
              ...
              if(query.length != 0){                
              axios.get('/admin/api/blogs/search/'+query)
                        .then((response)=>{  
           ...
           getResults(page = 1) {
            ...
            axios.get('/admin/api/blogs?page=' + page)
                .then(({data}) => (
                    this.blogs = data
                ))
        },
